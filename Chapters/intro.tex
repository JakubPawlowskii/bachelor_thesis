...
\end{align}
where \(O_{mn} = \matrixel{m}{O}{n}\).
We immediately see that, assuming absence of degeneracy, long-time average of the off-diagonal
...
\begin{equation}
    \sum_{n}\abs{C_n}^2 O_{nn} \approx \bar{O}\sum_{n}\abs{C_n}^2 = \bar{O}
\end{equation}
obtaining a result consistent with the microcanonical ensemble, albeit without energy dependence
so equivalent to the infinite temperature limit. 
Furthermore, off-diagonal elements are exponentially
small in system size, so in order for the second term in average of~\eqref{eq:time evolution}
to vanish, it is necessary only for the matrix elements with largest contribution to the expected value to
average to zero. This can happen on much shorter timescales that would be required for eliminating
matrix elements between all eigenstates~\autocite{DAlessio2016}.
...
times should be observable dependent. These shortcomings were fixed in ground-breaking works
...
entropy at energy \(\bar{E}\). Both \(O(\bar{E})\) and \(f_{O}(\bar{E},\omega)\) are smooth
...
Numerical clues for the validity of ETH have been found in many correlated lattice models
...
the review by~\textcite{DAlessio2016}, on which this part of Introduction is based.
...
for which the ETH does not hold. There is no unique definition of quantum
...
This problem is our main interest in Chapter~\ref{chap:iom}, where 
following~\textcite{Mierzejewski2015a} we describe in detail an algorithm allowing for
...
Chapter~\ref{chap:iom} we also introduce the well-known concept of Mazur 
...
As an application of the introduced methodology, we study the energy current and
...
In such weakly perturbed quantum systems, the number of conserved quantities usually behaves like
in generic nonintegrable models, so integrals of motion from parent integrable model start to decay.
Investigating long-time behavior of the autocorrelation functions of
aforementioned energy current and two noncommuting observables 
in spin-\(1/2\) XXZ model is the subject of Chapter~\ref{chap:decay}. In case of
...
The remainder of this chapter is devoted to a short introduction
...
In this chapter, we deal with the former of two models.
First, a physics-motivated derivation of Heisenberg model is be presented. Afterwards,
on our way to XXZ model, we discuss the mathematical details and various modifications
of original model. Our review of this topic is based on the books by~\textcite{spalek2015} and 
...
\(\RR^3\), and \(\mathfrak{h}_i \cong \CC^2\) is the two-dimensional vector space spanned by
...
This is a crucial observation that leads to the development of Heisenberg model. We shall now
...
\end{align}
In general, we have that \(E^{(s)}\neq E^{(a)}\). We can argue that this is the case
with Coulomb potential, because it blows up to infinity at \(\bm{r}_1 \to \bm{r}_2\),
whereas the antisymmetric wave function vanishes at this point. It is thus energetically
favorable for our system to pick the one of the two possible wave functions.
...
exclusion principle that forces the choice of a given state. Let us now do something
...
\end{align}
which shall be of use in Appendix~\ref{app:orthonormality}.
...
\(\bm{S}^2 = (\bm{S}_1 + \bm{S}_2)^2\) for which our triplet (total spin \(s=1\)) and singlet
...
\paragraph{Equivalent Hamiltonian}After this detour into the quantum mechanics of spin, let us return to the problem
...
\begin{equation}
    J_{ij} = \begin{cases}
        J,\;\;\; i \textrm{ is a nearest neighbor of } j\\
        0,\;\;\; \textrm{otherwise}
    \end{cases}
\end{equation}
...
is that is possess \(SU(2)\) symmetry (rotation of spins), which causes
...
\begin{equation}
    ...
    \label{eq:XXZ}
\end{equation}
which is the most common form of the XXZ Hamiltonian. The isotropy could be restored
...
one-dimension~\autocite{Baxter1971,Baxter1972}. However, these solutions are notoriously difficult and
we do not use them in this thesis. Instead, when in need of concrete computations, we
