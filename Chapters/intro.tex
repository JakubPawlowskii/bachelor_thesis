...
entropy an energy \(\bar{E}\). Both \(O(\bar{R})\) and \(f_{O}(\bar{E},\omega)\) are smooth
...
In a generic system exhibiting ETH, after thermalization the expectation values
...
grand canonical one).
For example in the canonical ensemble \(\hat{\rho} \propto \mathrm{e}^{H/kT}\)
and \(\bar{O}\approx \tr(O \hat{\rho})\).
However, there exists a class of models, called \textit{integrable},
...
existence of an extensive number of local operators \(\set{\hat{I}_k}\) that commute with
the Hamiltonian and with each  other\footnote{For relevant definitions see Section~\ref{sec:prelim}.}.
A very brief review of integrability is presented in Section~\ref{sec:integrability}.
...
in nonintegrable ones (such as energy or momentum).
