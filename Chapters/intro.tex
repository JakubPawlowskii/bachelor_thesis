...
of some observable \(A\) under time-independent Hamiltonian \(H\) with
...
\begin{align}
    A(t) & =\matrixel{\psi(t)}{A}{\psi(t)}=\sum_{m, n} C_{m}^{*} C_{n} 
    \mathrm{e}^{i\left(E_{m}-E_{n}\right) t} A_{m n} \nonumber\\
    &=\sum_{n}\abs{C_{n}}^{2} A_{nn}+\sum_{n \neq m} C_{m}^{*} C_{n} 
    \mathrm{e}^{i\left(E_{m}-E_{n}\right) t} A_{m n}\label{eq:time evolution}
\end{align}
...
\textcolor{blue}{tu już o ETH}


However, the topic of this thesis is integrals of motion.
