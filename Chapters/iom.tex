...
motion (LIOMs and QLIOMs) supported on \(\NN \ni m \leq L/2\) sites in a model given by 1-D tight-binding Hamiltonian \(H\). To this end, we employ the algorithm first proposed
in~\textcite{Mierzejewski2015a}. In our description and notation used, we will  follow
...
\in \mathcal{V}_L^M\). One possible approach to checking its locality would be to
...
\begin{equation}
  K_{st} = \hs{\bar{O_s}^{\tau}}{\bar{O_t}^{\tau}}
\end{equation}
This matrix is symmetric by design, as considered operators are Hermitian and thus of the form
\(O+O^{\dagger}\) or \(i\left(O-O^{\dagger}\right)\). Therefore, the spectral theorem guarantees existence of an unitary matrix \(U\) that
diagonalizes it. In other words, \(D = UKU^{\dagger}\) is diagonal and we have the following relations:
\begin{align}
  &\sum_{s, t} U_{n s} K_{s t} U_{t m}^{T}=\delta_{nm} \lambda_{n} \in \RR,\;\;\; \lambda_n \text{ --- eigenvalue of }K \nonumber \\
  &UU^{\dagger} = U^{\dagger}U = \mathbb{1} \implies \sum_{s} U_{ns}U_{sm}^{T} = \delta_{mn}\nonumber\\
  & U K = D U \implies \sum_s U_{ns} K_{st} = \sum_s  \delta_{ns} \lambda_s U_{st} = \lambda_n U_{nt}\label{eq:properties}
\end{align} 
With the help of the \(U\) matrix (eigenvectors of \(K\)) we can define a new set of rotated operators that
...
\begin{proof}
  ...
  \begin{align*}
    ...
    = \sum_{t} \left(\sum_{s}U_{ns} K_{st}\right)  U_{tm}^{T} \\ 
...
about the eigenvalues of \(K\) matrix from a simple corollary to Proposition~\ref{proof:orthogonality}.
\begin{corollary}
...
that \(\left(\forall n\right) \left(\lambda_n \geq 0\right)\). Therefore K is a positive semi-definite matrix.
\end{proof}
This corollary provides us with a lower bound on spectrum of matrix \(K\). 
...
\begin{align}
  ...
  &= \sum_s  \left( \sum_t U_{n t}K_{ts}\right) O_{s}+Q_{n}^{\perp} = \sum_{s} 
  \lambda_{n} U_{n s} O_{s}+Q_{n}^{\perp} = Q_{n}^{M}+Q_{n}^{\perp}
\end{align}

\textcolor{blue}{TODO: finall results on support and proof of correctness}
