...
We will finish the section with an example of an application of Mazur bound. To this end, we can 
...
of the form:
\begin{equation}
  O_{\underline{s}}=\sum_{j=1}^L \sigma_{j}^{s_{1}} \sigma_{j+1}^{s_{2}} \cdots 
  \sigma_{j+m-1}^{s_{m}}
  \label{eq:basis operator}
\end{equation}
In the expression above we have \(\sigma_j^z \equiv \sqrt{2} S^z_j\),
\(\sigma_j^{\pm} \equiv S_j^{\pm}\), \(\sigma^{0} \equiv \Id{}\) and
 ...
 operator.\newline
 \textcolor{blue}{why no 1/L in my HS product? how it relates to slightly different
 definitions of sigmas? Operators constructed as in paper are normalized, here aren't}
As a matter of mathematical precision, the notation for \(O_{\underline{s}}\) used here 
...
\begin{equation}
  O_{\underline{s}}=\sum_{j=1}^L \underbrace{\Id \otimes \cdots
       \otimes \Id}_{j-1} \otimes \; \sigma_{j}^{s_{1}} \otimes \sigma_{j+1}^{s_{2}} \otimes
        \cdots \otimes \sigma_{j+m-1}^{s_{m}} \otimes 
        \underbrace{\Id \otimes \cdots \otimes \Id}_{L-j-m+1}
\end{equation}
The single site identity operators ensure that the dimension of matrix of the operator is right.
A simple combinatorial observation shows us, that there are \(N_m = 3\cdot 4^{m-2}\cdot 2\) operators
constituting a \(m\)-local basis (for \(m = 1\) we have \(N_1 = 3\)). Moreover, they are orthogonal (\textcolor{blue}{orthonormal?}) 
\textit{by design}, i.e.\ \(\hs{O_{\underline{s}}}{O_{\underline{k}}} \propto
\delta_{\underline{s},\underline{k}}\).
\newline
\textcolor{blue}{Here proof of orthogonality(orthonormality), maybe an Appendix?}
\newline
...
direct sum of linear spaces we now that its cardinality is \(D_M = \sum_{m=1}^{M} N_m = 
 3 \cdot 4^{M-1}\). Such construction can be implemented in practice by considering
...
precisely in the \textit{noncommuting} operators, i.e\ precisely those that break this symmetry.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Commuting LIOM: Spin energy current}
...
\end{equation}
As the energy current commutes with the Hamiltonian, it does not undergo time evolution
...
translational invariance. Available resources allowed us to make unrestricted search for
 ...
 \(L\) and \(\Delta \) turned out to be translationally invariant. Therefore, we restrict our further 
considerations to translationally invariant operators only. This allowed us to obtain numerical
...
parameter \(\Delta=1.0\) and \(\Delta=0.5\) respectively.

For the case of \(\Delta=1.0\), the XXZ model reduces to the isotropic XXX model possessing full
\(SU(2)\) symmetry:
\begin{equation}
  H_{XXX} = \frac{J}{2} \sum_{i=1}^L \left(\Sx_i \Sx_{i+1} + \Sy_i \Sy_{i+1} + \Sz_i \Sz_{i+1}\right)
\end{equation}
...
and \(\Sy_{tot}\) and therefore their linear combination, i.e.\ the following quantity:
\begin{equation}
    \hat{O}_{1} =  \frac{1}{\sqrt{L}}\sum_{i=1}^{L} \Sp_{i} + \text{H.c.}
    \label{eq:delta 1.0}
\end{equation}
where the prefactor is introduced for the sake of normalization. Operator \(\hat{O}_1\) is an example of a real operator.
...
that there exist an unitary operator \(U\), which action is equivalent to flipping the sign 
...
quantity for \(-\Delta\). Detailed discussed of this topic is far beyond the scope of this thesis
and can be found in~\autocite{zadnik2016,Prosen2014c}. It turns out, that for \(\Delta=0.5\) the actions of this operator
produces a simple factor of \((-1)^i\) in the integral of motion:
\begin{equation}
  \hat{O}_{2} =\frac{1}{\sqrt{L}} \sum_{i=1}^{L} {(-1)}^i \left( \Sp_{i}\Sp_{i+1}\Sp_{i+2}\right) + \text{H.c.}
  \label{eq:delta 0.5}
\end{equation}  
This is once again a real operator.
...
Both \(\hat{O}_1\) and \(\hat{O}_2\) are noncommuting (Q)LIOMs, what can be seen easily from the
fact that they consist of just \(\Sp\) operators (not counting the Hermitian conjugate). Conducting
...
we see that \(\hat{O}_1\) is a local integral of motion, whereas \(\hat{O}_2\) is a quasilocal integral of motion. 