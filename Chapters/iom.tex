...
\begin{figure}[!htbp]
  ...
  \end{tikzpicture}
  \caption{Illustration of a \(m\)-local operator. Blue circles correspond to single-site
  identity operators, green circles to an arbitrary single-site operator except for the identity and
  red circles to an arbitrary single-site operator. If we were to allow identity operators on
  sites marked with green, we would have obtained a \((m-1)\)-local operator.}\label{fig:1D chain}
\end{figure}
...
It is important not to lose the physical interpretation of these results amidst the formal development.
...
\begin{example}
  ...
  we have the following expression for the real (non-dissipative) part of dynamical
...
A simple combinatorial observation shows us that there are \(N_m = 3^2\cdot 4^{m-2}, \; m \geq 2\)
...
We conclude this chapter with a short summary of obtained noncommuting (Q)LIOMs.
