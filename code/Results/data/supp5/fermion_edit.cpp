...
{
    constexpr uint sites = var1;     // no. of sites
    uint states = pow(2, sites);  // no. of states, 2^sites
    constexpr double delta = var2; // delta in Heisenberg model
    constexpr double t = var3;
...
namespace operators
{
    ...
    uint max_supp = 5;                                         // maximal support of operators
...
arma::mat generate_operator_matrix(int);
...
int main()
{
    ...
    for (uint i = 0; i < no_op; i++)
    {
        arma::mat op1 = generate_operator_matrix(i);
        for (uint ii = 0; ii < no_op; ii++)
        {
            arma::mat op2 = generate_operator_matrix(ii);
            corr_mat(i, ii) = arma::trace(op1.t() * op2) / states;
        }
    }
    // corr_mat.clean(1e-16);
    // corr_mat.save("../corr_mat_cpp.txt", arma::raw_ascii);
    /*
        Clear eigenval and eigenvec as they are no longer needed,
        corr_mat has been generated at this point.
        Reuse them form eigenvectors and eigenvalues of corr_mat.  
    */
    eigenval.clear();
    eigenvec.clear();
...
arma::mat generate_operator_matrix(int index)
{
    /*
