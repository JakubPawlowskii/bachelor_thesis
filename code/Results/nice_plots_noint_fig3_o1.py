...
def get_lambdas(t,delta,L,mode,alpha,taus,idx):
    ...
    for i,l in enumerate(L):
        ...
        fixed_tau_name = 'data/preprocessed/spin_new/' + mode + '_fixed_tau_L_' + str(l) + '_d_' + '{:.1f}'.format(delta) + '_a_' + '{:.2f}'.format(alpha) + '.csv'
...
def plot_lambdas(ax,t,delta,mode,alpha,taus,taus_goal,idx):
    ...
    for i in range(len(taus)):    
        ...
        if taus[i] < 1e6:
            ...
            label = r'$\omega = 0^{+}$'
        pp = ax.plot(inv_L,lambda_noint[:,i],symbols[i],label=label,markersize = 9, markeredgewidth=.75, markeredgecolor='k',linewidth=0.8)
    ...
    pp = ax.plot(inv_L,lambda_int,symbols[len(taus)],label=label,markersize = 9, markeredgewidth=.75,linewidth=0.8,markeredgecolor='k')
...
latex_plot(scale = 1, fontsize=18)
...
alpha = 0.05
plot_lambdas(ax1,t,delta,mode,alpha,taus,taus_goal,idx)

ax1.text(0.88,0.92,r'(a)',horizontalalignment='left',verticalalignment='center', transform=ax1.transAxes,zorder=6, fontsize=textsize)
ax1.text(0.3,0.12,r'$\Delta ='+'{:.2}'.format(delta)+r'$',horizontalalignment='left',verticalalignment='center', transform=ax1.transAxes,zorder=6, fontsize=textsize)
ax1.text(0.3,0.12-0.05,r'$\alpha = '+'{:.2}'.format(alpha)+r'$',horizontalalignment='left',verticalalignment='center', transform=ax1.transAxes,zorder=6)
...
alpha = 0.1
plot_lambdas(ax2,t,delta,mode,alpha,taus,taus_goal,idx)

ax2.text(0.88,0.92,r'(b)',horizontalalignment='left',verticalalignment='center', transform=ax2.transAxes,zorder=6, fontsize=textsize)
ax2.text(0.3,0.12,r'$\Delta ='+'{:.2}'.format(delta)+r'$',horizontalalignment='left',verticalalignment='center', transform=ax2.transAxes,zorder=6, fontsize=textsize)
ax2.text(0.3,0.12-0.05,r'$\alpha = '+'{:.2}'.format(alpha)+r'$',horizontalalignment='left',verticalalignment='center', transform=ax2.transAxes,zorder=6)
...
alpha = 0.2
plot_lambdas(ax3,t,delta,mode,alpha,taus,taus_goal,idx)
# ax3.set_ylabel(r'$\lambda_1$',fontsize=20)
ax3.text(0.88,0.92,r'(c)',horizontalalignment='left',verticalalignment='center', transform=ax3.transAxes,zorder=6, fontsize=textsize)
ax3.text(0.45, 0.8,r'$\Delta ='+'{:.2}'.format(delta)+r'$',horizontalalignment='left',verticalalignment='center', transform=ax3.transAxes,zorder=6)
ax3.text(0.45,0.8-0.05,r'$\alpha = '+'{:.2}'.format(alpha)+r'$',horizontalalignment='left',verticalalignment='center', transform=ax3.transAxes,zorder=6)
...
for ax in axes:
    if ax == ax3:
        # ax.legend(loc='center left',fontsize=legend_size,frameon=False,bbox_to_anchor=(0.0,0.65))
