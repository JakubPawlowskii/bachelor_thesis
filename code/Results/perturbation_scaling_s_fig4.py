...
def get_R1_su2(t,delta,L,mode,alpha,taus,idx)->np.ndarray:
    ...
    for i,l in enumerate(L):
        for j,a in enumerate(alpha):
            ...
            fixed_tau_name = 'data/preprocessed/su2_breaking/delta'+str(delta)+'/fixed_tau_L_' + str(l) + '_d_' + '{:.2f}'.format(delta+a) + '_a_' + '{:.2f}'.format(0.00) + '.csv'
...
# scaling_factors = [1.2,1.2,1.5,1.5]
...
fit_and_plot(ax1,fun,x_flat,R1_flat,'k','-',1.5,scaling=scaling_factor)
...
fit_and_plot(ax2,fun,x_flat,R1_flat,'k','-',1.5,scaling=scaling_factor)
...
fig_name = 'fig4_small_alpha_1.1_arctan'
