...
def get_R1_su2(t,delta,L,mode,alpha,taus,idx)->np.ndarray:
    ...
    for i,l in enumerate(L):
        for j,a in enumerate(alpha):
            ...
            fixed_tau_name = 'data/preprocessed/su2_breaking/delta_0.5/fixed_tau_L_' + str(l) + '_d_' + '{:.2f}'.format(delta+a) + '_a_' + '{:.2f}'.format(0.00) + '.csv'
...
mode = 's'
R1 = get_R1_su2(t,0.5,[14],mode,alpha,taus,idx_su2)
x = np.zeros((len(alpha),len(taus)))
for i,a in enumerate(alpha):
    x[i,:] = 1/(taus*(np.abs(a)**scaling_factor))
...
for i,ax in enumerate(axes):
    ...
    if ax == ax4 or ax == ax3 or ax == ax1 or ax == ax2:
