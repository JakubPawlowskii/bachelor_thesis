...
def plot_R1(ax,R1,x,alpha,mode,markers):
    for i,a in enumerate(alpha):
        ...
        if mode == 'ec':
            ...
            # ax.plot(x[i,:],R1[i,:],'x',markersize=markersize,label=label,markevery=markevery)
        else:
...
fig.suptitle(r'Noncommuting LIOM, $\Delta\pm\alpha$')

# scaling_factors = [1.2,1.2,0.75,0.75]
...
alpha = [-0.5,-0.4,-0.3,-0.2]
# alpha = [-0.1,-0.05,0.05,0.1]
idx = (taus_all < 10000) & (taus_all > 1) 
#===================== Left upper panel ============================================
...
for i,a in enumerate(alpha):
    x[i,:] = 1/(taus*(abs(a)**scaling_factor))
...
# fig_name = 'R1_spin_su2_breaking_large_pos_alpha_best_arctan_fit'
