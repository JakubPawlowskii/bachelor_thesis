...
def fit_and_plot(ax,fun,x,R1,col,ls,lw):
    ...
    xx = np.linspace(x_flat[0],max_x,100)
    # popt[0] = 0.4
    y = fun(xx,*popt)
    ax.plot(xx,y,ls,color=col,linewidth=lw,label=r'$2/\pi \; \arctan\left[\omega/(' + r'{:.3f}'.format(popt[0])+ r'\alpha^2)\right]$')
...
alpha = [0.3,0.4,0.5]
# alpha = [0.6,0.7,0.8]
idx = (taus_all < 100000000000000000000) & (taus_all > 0) 
#===================== Left upper panel ============================================
scaling_factor = 2
# alpha = [0.6]
taus = taus_all[idx]
...
# ax1.text(0.7,0.6-0.07,r'$L = 14$',horizontalalignment='left',verticalalignment='center', transform=ax1.transAxes,zorder=6)
...
# ax1.set_xlabel(r'Frequency $\omega$',fontsize=labelssize)
# #================ Right upper panel ======================================
...
# ax2.set_xlabel(r'Frequency $\omega$',fontsize=labelssize)
# #================ Left lower panel ======================================
# scaling_factor = 0
# # alpha = [0.3,0.4,0.5,0.6,0.7,0.8,0.9]
# # idx = (taus_all < 10000) & (taus_all > 1.5) 
...
# sl1 = slice(0,midpoint1,2)
# sl2 = slice(midpoint1+1,midpoint1+midpoint2,8)
# sl3 = slice(midpoint1+midpoint2+1,-1,15)
...
# ax4.text(text_x+0.05,text_y-0.07,r'extrap. from',horizontalalignment='center',verticalalignment='center', transform=ax4.transAxes,zorder=6)
# ax4.text(text_x-0.06,text_y-0.12,r'$L=11,12,13,14$',horizontalalignment='left',verticalalignment='center', transform=ax4.transAxes,zorder=6)
letters = ['(a)','(b)','(c)','(d)','(e)','(f)','(g)','(h)','(i)']
txt1 = [r'$\Delta = 1.0$',r'$\Delta = 1.0$']
txt2 = [r'$L = 14$',r'$L\to \infty $']
for i,ax in enumerate(axes):
    
    if ax == ax1 or ax == ax2:
        ax.text(0.7,0.6,txt1[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6)
        ax.text(0.7,0.6-0.1,txt2[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6)
        ax.text(0.55,0.6-0.035,letters[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6, fontsize=textsize+4)
    # elif ax == ax3 or ax == ax4:
    #     ax.text(0.7,0.6,txt1[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6)
    #     ax.text(0.7,0.6-0.07,txt2[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6)
    #     ax.text(0.6,0.6-0.035,letters[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6, fontsize=textsize+4)
...
fig_name = 'current_scaling_small'
