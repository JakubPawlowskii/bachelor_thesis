...
def import_eigenvalues(L, t, d, type):
    ...
    folder_abs_path = '/home/jakubp/Code/work/ETH/qliom_all/results_qliom/data/supp3_trans_sym/'
    file_name = ''
...
def import_eigval_one_op(L, t, d, tau,  type):
    folder_abs_path = '/home/jakubp/Code/work/ETH/qliom_all/results_qliom/data/supp3_noint/'
    file_name = ''

    if type == 'f':
        file_name = 'fermion_L_'+str(L)+'_t_'+str(t)+'_delta_'+"{:.2f}".format(d)+'_tau_' + "{:.2f}".format(tau)+'.dat'
    elif type == 's':
        file_name = 'spin_L_'+str(L)+'_t_'+str(t)+'_delta_'+"{:.2f}".format(d)+'_tau_'+ "{:.2f}".format(tau)+'.dat'
    else:
        print('No such type')
        return 0
...
# L = [9,11,13]
delta = [-0.5]
tau = [20.00,50.00,200.00,100000000.00]
t = -0.5
types = ['s']
style = [':o',':s']
size = [7,6]
...
for ind3, type in enumerate(types):
    ...
    for ind1, d in enumerate(delta):
        if type == 'f':
            label = r'Fermion $\Delta = ' + str(d) + r',\; \alpha = 0.0,\; \tau = ' + r'\infty'+ r'$'
        if type == 's':
            label = r'Spin $\Delta = ' + str(d) + r',\; \alpha = 0.0,\; \tau = ' + r'\infty' +r'$'
        eigs = []
        LL = []
        ...
        # print(p[0], p[1])
        # print(inv_L)
        pp = plt.plot(inv_L, eigs, style[ind3],markersize = size[ind3], label=label)
        if d == 1.0 or d == -0.5:
...
for ind3, type in enumerate(types):
    label = ''
    for ind1, d in enumerate(delta):
        for ind4, tt in enumerate(tau):
            tau_str = r'\infty' if tt == 100000000 else str(tt)
            if type == 'f':
                label = r'Fermion $\Delta = ' + str(d) + r',\; \alpha = 0.09,\; \tau = ' + tau_str + r'$'
            if type == 's':
                label = r'Spin $\Delta = ' + str(d) + r',\; \alpha = 0.09,\; \tau = ' + tau_str +r'$'
            eigs = []
            LL = []
            for ind2, l in enumerate(L):
                # print(l)
                try:
                    eigval = import_eigval_one_op(l, t, d, tt, type)
                    # eigval = import_eigenvalues(l,t,d,type)
                    # print(eigval[-1])
                    # print(eigval)
                    eigs.append(eigval)
