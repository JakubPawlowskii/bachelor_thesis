...
def plot_R1(ax,R1,x,alpha,mode,markers):
    for i,a in enumerate(alpha):
        ...
        if mode == 'ec':
            ...
            ax.plot(x[i,:],R1[i,:],marker,markersize=markersize,label=label,markevery=markevery,markeredgecolor='k')
        else:
            label = r'$\alpha = ' + r'{:.2f}'.format(a) + r'$'
            ax.plot(x[i,:],R1[i,:],marker,markersize=markersize,label=label,markevery=markevery,markeredgecolor='k',markeredgewidth=0.4)
...
# x = np.concatenate((x[:,sl1],x[:,sl2],x[:,sl3]),axis=1)
# R1 = np.concatenate((R1[:,sl1],R1[:,sl2]#================ Left lower panel ======================================
...
# ax4.text(text_x-0.06,text_y+0.05,r'$\Delta = 1.0$',horizontalalignment='left',verticalalignment='center', transform=ax4.transAxes,zorder=6)
