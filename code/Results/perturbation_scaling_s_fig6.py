...
# scaling_factors = [1.2,1.2,1.2,1.2]
scaling_factors = [1.0,1.0,1.0,1.0]
# scaling_factors = [0.75,0.75,0.75,0.75]
...
R1 = get_R1(t,-0.5,[11],mode,alpha,taus,idx)
...
taus = taus_all[idx]
mode = 's'
R1 = get_R1(t,-0.5,[13],mode,alpha,taus,idx)
x = np.zeros((len(alpha),len(taus)))
for i,a in enumerate(alpha):
    x[i,:] = 1/(taus*(np.abs(a)**scaling_factor))
...
fig_name = 'fig6_delta_-0.5_alpha_1.0_small'
