...
taus = taus_all[idx]
R1 = get_R1(t,1.0,[10,12,14,16],mode,alpha,taus,idx)
x = np.zeros((len(alpha),len(taus)))
for i,a in enumerate(alpha):
    x[i,:] = 1/(taus*(a**scaling_factor))
...
taus = taus_all[idx]
# mode = 'ec'
R1 = get_R1(t,0.5,[10,12,14,16],mode,alpha,taus,idx)
x = np.zeros((len(alpha),len(taus)))
for i,a in enumerate(alpha):
    x[i,:] = 1/(taus*(a**scaling_factor))
...
for i,ax in enumerate(axes):
    ...
    if ax == ax1 or ax == ax2:
        ...
        ax.text(0.7,0.3,txt1[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6)
        ax.text(0.7,0.3-0.1,txt2[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6)
        ax.text(0.55,0.3-0.035,letters[i],horizontalalignment='left',verticalalignment='center', transform=ax.transAxes,zorder=6, fontsize=textsize+4)
        
    if ax == ax3 or ax == ax4:
        if scaling_factor == 2:
            ax.set_xlabel(r'$\omega/\alpha^{2}$',fontsize=labelssize)
        if scaling_factor == 1:
            ax.set_xlabel(r'$\omega/\alpha$',fontsize=labelssize)
        else:
            ax.set_xlabel(r'Frequency $\omega$',fontsize=labelssize)
            # ax.set_xlabel(r'$\omega/\alpha^{2}$',fontsize=labelssize)
