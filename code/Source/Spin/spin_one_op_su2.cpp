...
void save_op_for_ham(arma::vec , operators_data *, xxz_real *);
int main(int argc, char **argv)
{
    ...
    arma::vec op_coeffs;
    op_coeffs.zeros(ops.no_op);
    if (std::abs(delta-1.0) < 1e-10) op_coeffs[0] = 1.0;
    else if (std::abs(delta-0.5) < 1e-10) op_coeffs[6] = 1.0;
    
    std::vector<double> alphas = {0.05,0.1,0.2,0.0};
    xxz_real* ham2;
    for(double alpha : alphas)
    {
        ham2 = new xxz_real(ops.sites, t, delta+alpha, 0.0);
        save_op_for_ham(op_coeffs, &ops, ham2);
        delete ham2;            
    }
    ham2 = nullptr;
...
void save_op_for_ham(arma::vec ops_coeffs, operators_data *ops, xxz_real *ham)
{
    if (!ham->is_diag())
    {
        std::cout << "Diagonalizing Hamiltonian for su2, delta = " << ham->get_delta() << std::endl;
        ham->diagonalize();
        ham->clear_ham_matrix();
    }
    std::cout << "# Saving operator for su2, delta " << ham->get_delta() << std::endl;
    arma::mat opmat = generate_op_matrix_from_vector(ops_coeffs, ops, ham, 1e+8);
