...
namespace parameters
{
    ...
    constexpr double alpha = 0.2;    //var4 // if alpha != 0, then integrability is broken
...
int main()
{
    ...
    for (uint i = 0; i < no_op; i++)
    {
        arma::mat op1 = generate_op_matrix(i, epsilon);
        for (uint ii = 0; ii < no_op; ii++)
        {
                arma::mat op2 = generate_op_matrix(ii, epsilon);
            corr_mat(i, ii) = arma::trace(op1.t() * op2) / static_cast<double>(states);
        }
    }
    
    /*
        Clear  eigenvec as they are no longer needed,
        corr_mat has been generated at this point.
        Reuse them for eigenvectors  of corr_mat.
        Hamiltonian eigenvalues are still needed.
    */
    eigenval.zeros(no_op);
    eigenvec.zeros(no_op,no_op);
    arma::eig_sym(eigenval, eigenvec, corr_mat);

    cout << "#Correlation matrix eigenvalues: " << endl;
    for(double val : eigenval) cout << std::setprecision(17) << val << endl;
    ...
    // arma::mat op = generate_op_from_eigenvector(1,1e-8);
    ...
    // out = arma::trans(out);
    // out.save("out4.csv", arma::csv_ascii);
...
arma::mat generate_op_from_eigenvector(int index, double eps)
{
    ...
    {
        if(std::abs(vec(i)) < 1e-8) continue;
