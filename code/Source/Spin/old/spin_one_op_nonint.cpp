...
namespace parameters
{
    ...
    constexpr double alpha = 0.00;    
    constexpr double t = -0.5;       //var3
    constexpr double tau = 1e+8; //var4// averaging 'time'
...
if ((sites == 8) && delta < 0.0)
    ...
    if ((sites == 9) && delta < 0.0)
    {
        op_def = append_ops(54, 62, op_def);

        coeffs_vals = {-1.0/std::sqrt(9.0)};
        coeffs.resize(coeffs_vals.size() * sites);
    ...
    if ((sites == 11) && delta < 0.0)
    {
        op_def = append_ops(66, 76, op_def); 

        coeffs_vals = {-1.0/std::sqrt(11)};
        coeffs.resize(coeffs_vals.size() * sites);
    ...
    if ((sites == 13) && delta < 0.0)
    {
      
        op_def = append_ops(78, 90, op_def); 
       

        coeffs_vals = {1.0/std::sqrt(13.0)};
        coeffs.resize(coeffs_vals.size() * sites);
    ...
    if ((sites == 15) && delta < 0.0)
    {
        op_def = append_ops(90, 104, op_def); 
        

        coeffs_vals = {1.0/std::sqrt(15.0)};
        coeffs.resize(coeffs_vals.size() * sites);
