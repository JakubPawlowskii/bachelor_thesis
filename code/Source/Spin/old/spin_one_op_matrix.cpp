...
{
    constexpr uint sites = 16;        //var1     // no. of sites
...
int main()
{
    ...
    std::vector<int> op = {0, 1, 1, 1};
    std::string filename = "111";
    // cout<<find_op_index(op)<<endl;
    arma::vec opmat = generate_op_matrix(find_op_index(op), epsilon).as_col();

    unsigned long long length = opmat.n_rows;
    unsigned long long length1 = length / 4;
    unsigned long long length2 = 2 * length1;
    unsigned long long length3 = 3 * length1;
    ...
    file1.open(filename + "_1.txt");
    file1 << std::setprecision(17);
    for (unsigned long long i = 0; i < length1; i++)
    {
        file1 << opmat(i) << std::endl;
    }
    file1.close();

    file2.open(filename + "_2.txt");
    file2 << std::setprecision(17);
    for (unsigned long long i = length1; i < length2; i++)
    {
        file2 << opmat(i) << std::endl;
    }
    file2.close();

    file3.open(filename + "_3.txt");
    file3 << std::setprecision(17);
    for (unsigned long long i = length2; i < length3; i++)
    {
        file3 << opmat(i) << std::endl;
    }
    file3.close();

    file4.open(filename + "_4.txt");
    file4 << std::setprecision(17);
    for (unsigned long long i = length3; i < length; i++)
    {
        file4 << opmat(i) << std::endl;
    }
    file4.close();
...
arma::mat generate_op_matrix(int index, double eps)
{
    ...
    */
    ...
    for (uint j = 0; j < states; j++)
    {
        for (uint jj = 0; jj < states; jj++)
        {
            if (std::abs(eigenval(j) - eigenval(jj)) > eps)
            {
                op(j, jj) = 0.0;
            }
        }
