...
int generate_hamiltonian_dense()
{
    ...
    arma::eig_sym(eigenval_ham, eigenvec, hamiltonian);
...
arma::mat generate_current_matrix(int index)
{
    ...
    */
    ...
    for (uint j = 0; j < states; j++)
    {
        ...
        {
            if (std::abs(eigenval_ham(j) - eigenval_ham(jj)) > epsilon)
            {
                op(j, jj) = 0.0;
            }
        }
